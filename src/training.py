...
def train_gae(data, model, hyperparameters, lr):
    ...
    # ========== Initialize enhancement modules ONCE (memory optimization) ==========
    # Get feature dimensions via initial forward pass
    with torch.no_grad():
        pos_mask_init = cell_train_data.edge_label == 1
        z_init, _, _, z_gene_init = model.encode(
            cell_train_data.x, 
            gene_train_data.x, 
            cell_train_data.edge_label_index[:, pos_mask_init], 
            gene_train_data.edge_index
        )
        cell_dim = z_init.shape[1]
        gene_dim = z_gene_init.shape[1]
    
    # Initialize Channel Enhancement Convolution Blocks
    cecb_cell = CECB(cell_dim)
    cecb_gene = CECB(gene_dim)
    
    # Initialize Spatial Enhancement Block
    seb_gene = Global(gene_dim, gene_dim)
    
    # Initialize Embedding Enhancement Block
    eeb_cell = EEB_stage2(cell_dim, cell_dim)
    
    # Initialize Triple Enhancement Fusion layers
    linear1 = nn.Linear(gene_dim, 16)
    relu = nn.ReLU()
    linear2 = nn.Linear(16, gene_dim)
   
    seb_heads = 8  # GlobalEnhancementBlock heads
    seb_intermediate_dim = seb_heads * gene_dim  # = 256
    linear_align = nn.Linear(seb_intermediate_dim, gene_dim)
    
    # Initialize Contrastive Loss
    contrastive_loss_fn = ContrastiveLoss(num_cells // 2)
    
    # Create unified optimizer with ALL trainable parameters
    all_parameters = (
        list(model.parameters()) +
        list(cecb_cell.parameters()) +
        list(cecb_gene.parameters()) +
        list(seb_gene.parameters()) +
        list(eeb_cell.parameters()) +
        list(linear1.parameters()) +
        list(linear2.parameters()) +
        list(linear_align.parameters())
    )
    ...
    with trange(num_epochs, desc="") as pbar:
        for epoch in pbar:
            ...
            # Update learning rate (adaptive schedule)
            current_lr = compute_adaptive_learning_rate(epoch, lr)
            for param_group in optimizer.param_groups:
                param_group['lr'] = current_lr

            # ============== CECB: Cell-level Enhancement Block ==============
            # Store original embeddings for contrastive learning
            z_cell_original = z_combined.clone()
            z_gene_original = z_gene.clone()
            
            # Apply Channel Enhancement Convolution Block
            z_combined = cecb_cell(z_combined)
            z_gene = cecb_gene(z_gene)
            z_gene_cecb = z_gene.clone()  # Store CECB output for fusion
